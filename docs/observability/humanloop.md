...
-d '{'model': '{{openai_small}}', 'messages': <YOUR HUMANLOOP PROMPT TEMPLATE>}'
```
...
```python
litellm.completion(
    model="humanloop/{{openai_small}}", # or `humanloop/anthropic/{{anthropic}}`
    # ...
)
```
...
```bash
{
  ...
  "model": "{{openai_small}}" # your template model
}
```
