...
response = litellm.completion(model="{{openai_small}}", messages=[{"role": "user", 
...
response = litellm.completion(model="{{openai_small}}", messages=[{"role": "user", "content": "Hey, how's it going?"}])
```
